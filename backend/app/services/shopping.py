...
def shopping_list(days: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    notes = set()
    for day in days:
        for entry in [*day["meals"], *day["fruits"], *day["snacks"]]:
        ...
        if any(not meal.get("ingredients") for meal in day["meals"]):
    ...
    notes.update(
        [
            "Quantities cover one person, including listed fruit and optional snacks; round up to pack sizes.",
            "For cooked grain and legume weights, buy ready-cooked grains or cooked/drained pulses.",
            "Arrange delivery by Monday. Choose frozen options or freeze suitable later-week ingredients on arrival.",
            "Salt, pepper and water to taste; check these kitchen basics before ordering.",
        ]
    )
...
def shopping_ingredients_for_extra(db: Session, item: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    try:
        return [ingredient_quantity({"name": item["name"], "quantity": label})]
    except ValueError:
