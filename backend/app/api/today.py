    ...
    try:
        ...
        if isinstance(exc, AIConfigurationError):
            raise HTTPException(
                ...
                detail="Food analysis requires a configured AI provider. Nothing was changed.",
            ) from exc
...
def analyze_workout_log(
    ...
    try:
        ...
        if isinstance(exc, AIConfigurationError):
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail="Workout analysis requires a configured AI provider. Nothing was changed.",
            ) from exc
